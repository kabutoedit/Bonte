...
export default function LeftNavBar() {
	...
	const fetchPage = useCallback(async (slug: string): Promise<CachedPage> => {
		if (fetchCacheRef.current[slug]) return fetchCacheRef.current[slug]
		try {
			const response = await axios.get(
				`https://back-bonte.anti-flow.com/api/v1/catalog/page/${slug}/`
	...
	const ensureCategorySubcategories = useCallback(
		async (categoryId: number, categorySlug: string) => {
			if (subCategories[categoryId]?.length) return subCategories[categoryId]
			const page = await fetchPage(categorySlug)
			setSubCategories(prev => ({ ...prev, [categoryId]: page.children || [] }))
			return page.children || []
		},
		[subCategories, fetchPage]
	)
	...
	const toggleCategory = useCallback(
		...
		[openCategories, ensureCategorySubcategories]
	)
	...
	}, [location.pathname, categories, ensureCategorySubcategories, fetchPage])
	...
	const selectedSubcategories = useMemo(() => {
		...
		return subCategories[selectedCategoryId] || []
